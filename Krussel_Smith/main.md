...
```math R(K_t,H_t,Z_t) = \alpha Z_tK_t^{\alpha-1}H_t^{1-\alpha}+1-\delta ```math

```math Z \in \{0.99,1.01\} ```math
...
```math \ln K'= b_{0,g} + b_{1,g}\ln K ```math 
and 
```math \ln H
'= d_{0,g} + d_{1,g}\ln K```math 

when $Z=1.01$

```math \ln K'= b_{0,b} + b_{1,b}\ln K ```math 
and ```math \ln H'= d_{0,b} + d_{1,b}\ln K ```math 
...
```mathV(a,e,K,H,Z) = max_{c,a',n} \frac{\left(c^\eta(1-n)^{1-\eta}\right)^{1-\mu}}{1-\mu} + 
\beta E\left[V(a',e',K',H',Z')|e,K,H,Z\right] ```math

Subject to the budget constraint:

```mathc \leq R(K,H,Z)a -a'+w(K,H,Z)e n ```math
...
```mathu_c(c,n) = \beta E[ R(K',H',Z')u_c(c',n')| e,K,H,Z]```math
...
```mathc = \left[ \frac{RHS}{\eta} \left( \frac{(1-\eta)}{\eta w(K,H,Z)}\right)^{-(1-\mu)(1-\eta)}\right]^\frac{-1}{\mu} ```math
...
```math ln K_{t+1} = 0.114 + 0.953ln K_t```math
$R^2 =0.999$ in bad times and:
```math ln K_{t+1} = 0.128 + 0.949ln K_t```math
...
```math ln K_{t+1} = 0.123 + 0.951ln K_t```math

For labor:
```math ln H_{t} = -0.560 - 0.260ln K_t```math
$R^2 =0.989$ in bad times and
```math ln H_{t} = -0.485 - 0.267ln K_t```math
$R^2 =0.992$ in good times times. Krusell & Smith results are:
```math ln H_{t} = -0.592 - 0.252ln K_t```math
in bad times and
```math ln H_{t} = -0.544 - 0.252ln K_t```math
in good times.
